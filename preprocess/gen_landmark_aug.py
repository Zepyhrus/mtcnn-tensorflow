...
# In[6]:
import sys
sys.path.extend(['preprocess', 'data', 'train', 'model'])
...
#%%
'''用于处理带有landmark的数据'''
size = parse_arguments(sys.argv[1:]).input_size
# 是否对图像变换
# Using Celeba data, augmetation is not necessary
argument = False
if size == 12:
  net = 'PNet'
elif size == 24:
...
for (imgPath, box, landmarkGt) in tqdm(data):
  ...
  if argument:
    ...
    for i in range(10):
      ...
      iou_cal = iou(crop_box, np.expand_dims(gt_box, 0))
      #只保留pos图像
      if iou_cal > 0.65:
  ...
  for i in range(len(F_imgs)):
    ...
    f.write(os.path.join(dstdir, '%d.jpg' %
                          (image_id))+' -2 '+' '.join(landmarks)+'\n')
    image_id += 1
