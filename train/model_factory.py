...
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
  ...
  with tf.variable_scope('PNet'):
    with slim.arg_scope([slim.conv2d], activation_fn=prelu,
      ...
      if training:
        cls_prob = tf.squeeze(
          conv4_1, [1, 2], name='cls_prob')  # [batch,2]
        cls_loss = cls_ohem(cls_prob, label)

        bbox_pred = tf.squeeze(
          bbox_pred, [1, 2], name='bbox_pred')  # [bacth,4]
        bbox_loss = bbox_ohem(bbox_pred, bbox_target, label)

        landmark_pred = tf.squeeze(
          landmark_pred, [1, 2], name='landmark_pred')  # [batch,10]
...
def R_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
  ...
  with tf.variable_scope('RNet'):
    with slim.arg_scope([slim.conv2d],
      ...
      #TODO: add dropout, dropout has no impact on inference period of the net
...
def O_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
  ...
  with tf.variable_scope('ONet'):
    with slim.arg_scope([slim.conv2d],
      ...
      fc_flatten = slim.flatten(net)
      #TODO: modify the default relu to prelu for the fully connection layer
      fc1 = slim.fully_connected(
        fc_flatten, num_outputs=256, scope='fc1', activation_fn=prelu)
