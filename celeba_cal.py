...
#%% generate boxes
# sub_labels = labels.sample(labels.shape[0] // 200)
sub_labels = labels

# copying images from celeba to picture folder
...
#%%
#选用图片
celeba_landmark_list = open('data/trainImageList.txt', 'w')
err_idx = 0
...
# plt.imshow(img_labeled[:, :, ::-1])
  ...
  # for j in range(5):
  #   cv2.circle(img_labeled,
  #     center=(int(sub_labels.iloc[i, -10+2*j]), int(sub_labels.iloc[i, -9+2*j])),
  #     radius=0, color=(0, 255, 0), thickness=3)

  # cv2.imwrite(des_img, img_labeled)
  # time.sleep(0.001)
